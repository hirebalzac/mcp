...
export function registerBriefingTools(server: McpServer) {
  ...
  server.tool(
    'create_briefing',
    'Create a briefing to write an article on a given topic. Costs 5 credits. By default writing starts immediately; set start_writing=false to queue the article for later writing based on the workspace schedule.',
    {
      workspace_id: z.string().describe('Workspace UUID'),
    ...
    async ({ workspace_id, start_writing, ...params }) => {
      ...
      if (start_writing === false) payload.start_writing = false;

      const res = await client.post(`/workspaces/${workspace_id}/briefings`, payload);
      return { content: [{ type: 'text' as const, text: JSON.stringify(res.data) }] };
    }
  );
}
